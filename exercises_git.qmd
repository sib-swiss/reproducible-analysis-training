...
## Making sure you are in a repository


In order to create commits, we need to make sure our directory is a git repository. 

We can check this in any directory by using either the terminal or Rstudio directly.

::: {.panel-tabset}

## terminal

Make your project directory your current directory with `cd` (if you click the **Terminal** tab in Rstudio you will directly in the project directory) and type:
...
In Rstudio, when you open a project with a git repository, a **Git** tab will be in the uppper right Rstudio tabset:

![a view of the Git tab in the uppper right Rstudio tabset](img/rstudio_git_pane.png)

If this tab is absent, your directory is not set up for git.

::: callout-note

To solve this, **in the R console** you can use the following command:

```R
usethis::use_git()
``` 

It will ask you 2 questions: about committing the current files, and about restarting Rstudio interface to make the git pane appear.

**Say no to the first one, and yes to the second.**
...
Check out the untracked files in the terminal with `git status`. Do all these files need to be version-controlled? If not, add the files/directories you don't want to version control to the `.gitignore`
:::
...
The git panel lets you know and control which files and folder are staged for a commit.

![Rstudio git panel with a different file statuses](https://thinkr.fr/wp-content/uploads/git_etat_fichier.png)

In the image above, the columns **status** tells us about the file status (duh!).
The files can be:
...
And the will stand on the left or the right of the column, depending on whether they are staged for the next commit (left) or not (right).
...
Now that we're all set for our first commit we can start by staging the files by ticking the name of the files we want to add.

The next step will be the commit itself:

![Rstudio git panel with a highlight on the commit button](https://thinkr.fr/wp-content/uploads/git_commit1.png)

This will open a window where you can inspect the commit, write a commit message and validate it.
...
Once you have a PAT, you can use it as password or, setup Rstudio to use it by calling `gitcreds::gitcreds_set()` and pasting your PAT when prompted. 
...
In my tests before the course, it was possible to make a mess if you do not ensure your branch is called `main` before you set up the remote repository. So please ensure you set it up correctly beforehand.

:::


![rstudio git panel](img/rstudio_git_panel_branch_highlight.png)

After clicking on this, you should see a new window appear:

![rstudio new branch panel](img/rstudio_new_branch.png)

Name the remote `origin`, and enter your repository url in the remote URL field.

![rstudio add remote panel](img/rstudio_new_remote.png)

Once you press the `Add` button you should see the following.

![rstudio new branch panel final state](img/rstudio_new_branch_final.png)

Make sure you have the `Sync branch with remote` ticked and click on `Create`

::: callout-note

If you do not sync the branch with the remote, then you will have to push the branch content yourself. 
...
Aternatively, using the Rstudio interface you can click on the pull button, and afterward switch branch (on the top-right) to the new branch.
