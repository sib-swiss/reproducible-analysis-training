...
The size and position of figures is quite well configurable with quarto. For example, you can quite easily create figure panels. 

::: callout-important
## Exercise
Check out the quarto documentation on [computed figures and cross-referencing](https://quarto.org/docs/authoring/cross-references.html#computations) and [custom layout](https://quarto.org/docs/authoring/figures.html#custom-layout). Create a panel with three figures:
...
Create two rows, and keep the two lines plots next to each other. On the second row let the heatmap span the entire column.

You can use this code in the chunk:
...
most_freq_girls |>
  plot_top(babynames, select_sex = "F", from = from_year, top = 5)

# plot top 10 girl names
most_freq_girls |>
  plot_top(babynames, select_sex = "F", from = from_year, top = 10)
...
The exercise is to add the hash-pipe options.
...
Add the follow to the top of the code chunk:
```{{r}}
#| label: fig-line-2010
#| layout: [[50,50], [100]]
